...
**上述配置项在安装后无法轻易更改。** 推荐将 `node-cidr-mask-size` 设置为 `/22`（每节点约 1024 个 Pod），或在需要时使用 `/20`（每节点约 4096 个 Pod）。继续增大（例如 `/16`）只会让单节点预留过大的地址空间，削弱集群扩展更多 worker 的能力。

上述配置仅修改了节点 Pod 使用的 IP 地址范围，如果需要修改节点的 Pod 数量上限，请参考下文。
...
在 k3s control-panel 所在的机器上，使用内置配置文件 `/etc/rancher/k3s/config.yaml` 调整参数：

- `sudo nano /etc/rancher/k3s/config.yaml`
- 增加或修改如下内容（示例将每节点上限提高到 800）：
...
- 可选：若你同时需要为每个节点预留更大的 Pod 子网，可在同一文件中配置 `kube-controller-manager-arg`。推荐使用 `/22`（约 1024 个 Pod/节点），或按需使用 `/20`：
...
- 注意：将 `node-cidr-mask-size` 设得过小（如 `/16`）会为单节点分配过大的地址块，降低集群横向扩展 worker 的灵活性。
