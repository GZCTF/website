...
- Optional: If you also need a larger Pod subnet per node, configure `kube-controller-manager-arg` in the same file. We recommend `/22` (~1024 Pods/node), or `/20` if required:
...
- Note: Using a very small mask (e.g., `/16`) allocates an oversized block per node and hurts horizontal scaling by limiting the number of workers you can add.
